	...
	if ($num_services == 0) {
		...
		if (isset($_SESSION['unblockip_results'])) {
			...
			if ( file_exists(dirname(__FILE__) . "/lang/". strtolower($lang.".php"))  ) {
				require_once(dirname(__FILE__) . "/lang/". strtolower($lang.".php"));
			}
			else {
...
add_hook('ClientAreaNavbars', 1, function ()
{
	...
	if (isset($client['userid']) && $_SESSION["unblockip_isactive"]) {
		$current_user = $client['userid'];
