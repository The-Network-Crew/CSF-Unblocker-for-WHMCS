...
function unblockip_output($vars) {
	...
	if ($_POST['action'] == "unblock") {
		if (!filter_var($_POST['ip_address'], FILTER_VALIDATE_IP)) {
			...
			if ($results['alerts'] != "") {
				$alerts .= $results['alerts'] . "<br>";
			}
			if (!$results['block_found']) {
				if ($_POST['server_id'] == "all") {
					$alerts .= "The IP " . $_POST['ip_address'] . " is not blocked on any active server.";  
				}
				else {
					$alerts .= "The IP " . $_POST['ip_address'] . " is not blocked on ". Capsule::table('tblservers')->find($_POST['server_id'])->name;  
				}
			}
		}
